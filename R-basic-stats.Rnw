...
Alternatively (though I do not see much of a gain here):
<<>>=
stripchart(myc ~ cond, vertical = TRUE, data = dmyc)
f1 <- function(x) {
    segments(x0 = 1, x1 = 2, 
...
have jumped from using \Rfunction{aov} to \Rfunction{lm}.
...
<<results='hide'>>=
library(MASS)
stepAIC(mcyst2, direction = "both")
stepAIC(mcyst, direction = "both")
stepAIC(longev_b_r, direction = "both")
stepAIC(metab_b_r, direction = "both")
@ 
