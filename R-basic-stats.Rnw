...
\item Look closely at the violin plots; what are the units of the width of the plot? Are you to add the widths if you were to think of a histogram/desntiy plot?
...
\item Do you remember what the formula for the t-statistic looks like? And
...
\item What a statistic is; estimators (for example, the sample mean is an
  estimator) are a type of statistic \footnote{A statistic is
    number that can be computed from a sample (without having to use unknown parameters). An estimator is a function
    used to calculate, or estimate, a quantity of a probability
    distribution. The sample mean, with the function $\sum x/N$, is a  function of the data and is used to estimate the true mean of a
    distribution using the data from a given sample. Thus, all estimators are statistics (or: estimator = statistic + specified target parameter), whereas not all statistics are necessarily estimators, in the sense that we don't use them to estimate a particular parameter; for example, we often use the t-statistic, as we will do in this class, as a statistic for testing, without mapping it to the parameter of anything ---though it could be used to estimate the non-centrality parameter of a t-distribution, and then it would play the role of a estimator.}.
...
\item We do not ``confirm the null hypothesis'': we fail to reject (and failing
  to reject can be something trivial to achieve ---carry out your  experiment with as tiny a sample size as possible)
...
The first scenario, the most common one, is called a two-tailed test. We sum both tails of the distribution. Deviations from \(H_0\) in both directions (A being larger than B, and A being smaller than B) count against the null.
...
Here is a interesting and recent example where the winner's curse can be
  ...
  such as reporting/publication bias and the reproducibility crisis.).
    ...
    Final note when we conduct many hypotheses tests (as in screening): using control of multiple testing (that we will see later) \textbf{does not} minimize or prevent the winner's curse (the winner's curse problem gets worse when we conduct many hypotheses tests).
...
For example, we might want to say ``We will only allow you to dump chlorine in the river if there is strong enough evidence that such action will not cause harm, for instance, will not increase fish mortality''. This is not something you can settle with p-values and null hypotheses as we have used them. \red{Why??}

What can we do? We want to turn the process around. For example, you would want a procedure for answering the following question: ``Is there strong enough evidence that, if chlorine has an effect, the effect is no larger than an increase in fish mortality of 1\%?''. This is like inverting the burden of proof: we want evidence in favor of a hypothesis that says that things are not worsened by more than a given, small, value, 1\% (i.e., it seems we now want evidence in favor of what is often the null). In other words, we want strong evidence that the true value is inside an acceptable region, ``things are similar, or not much worse than 1\%'' (we have simplified things here, by just worrying about increases in fish mortality, but often we worry about deviations both up and down).

What we can do is create a new null hypothesis, one that says ``Fish mortality increases by 1\% or more'' (where 1\% is the maximum tolerance in the increase of fish mortality). Unless we find strong evidence against that null, we will not proceed with the chlorine dumping.
If, instead, we can reject our new null, we have established that if chlorine increases fish mortality, it does not increase it by more than 1\%.

Now, what we have done is basically the following:
\begin{itemize}
\item Put the burden of proof on the ones that say ``chlorine is innocuous''. They can't just say ``I cannot reject the null that chlorine is not different from control''; they must show strong evidence of ``chlorine does not increase mortality by more than 1\%''.
\item That simply required using a one-tailed test. But \ldots
\item \ldots  there was this additional complicating factor of the 1\%:  it is not \(H_0: \mu_{chlorine} \ge \mu_{control}\);
  our null is \(H_0: \mu_{chlorine} \ge \mu_{control} + 1\% \)
\end{itemize}



%% FIXME
\red{FIXME: change the example along the following lines.}

Amodei says risk of extinction from AI is about 25\%, Musk about 30\% (get citations). Now, suppose someone (e.g., some politician, or some VC investor into AI) says: ``there is no evidence of risk of extinction from AI; what is your p-value?''. We can (should!) turn that around. We can thus say: ``I want strong evidence that risk of extinction is not larger than 1 in 100 million''\footnote{Where is the 1/100000000 figure from? This is a figure given by Stuart Russell in Steven Bartlett's podcast ``The diary of a CEO'' \Burl{https://stevenbartlett.com/doac/}, on 4 December 2025, around 1h 33' 30'' into the podcast (here is a link to the YouTube video and time \Burl{https://youtu.be/P7Y-fynYsgE?t=5610}). The logic is as follows: for nuclear plants we demand a risk no more than 1 in a million of a meltdown per year; we can also estimate the risk of extinction from a meteorite to be somewhere around 1 in 500 million to 1 in 1000 million (1 American billion) as a kind of background risk of extinction from other things. So, just to get the reasoning going, suppose we settle that we want the risk of extinction from AI to be no larger than 1 in a 100 million ---taking into account that human extinction is arguably much, much, much more serious than the meltdown of a nuclear plant, it does not seem exaggerated to demand a risk at least 100 times smaller, which is still larger than risks from meteorites.}.



The above would be an example of an ``inferiority'' test (there are also ``non-inferiority'' tests, that test the opposite, for example that a new treatment has not less effect  than a standard treatment).

A related cases is equivalence testing. On typical example involves comparing two drugs. Suppose the standard, widely used drug, to control for cholesterol is drug A (produced by ACME labs). Now, we want to assess if a given generic, drug B, is equivalent to the established drug A. For example, suppose we would allow pharmacies to sell drug B if drug B has effects that are within 1\% of the effects of A.

How can we do this?

We can approach this problem as one of finding evidence against a (new null) hypothesis that things differ by more of the specified tolerance, in our case that 1\% difference in effects; in other words, we set up a new null that says that the true value falls outside the equivalence bounds. If we can reject our new null that the groups differ by more than a given threshold (that the true difference falls outside the equivalence bounds), we have established that they are equivalent.  In some cases it is relatively straightforward to do this (such as with the TOST ---two one-sided tests --- procedure), in many others it is not.

Again, this is like inverting the burden of proof:  now we want evidence in favor of a hypothesis that says that things do not differ by more than a given, small, value (i.e., it seems we now want evidence in favor of what is often the null). In other words, we want strong evidence that the true value is inside the equivalence bounds, the bounds that say that ``things are similar, or equivalent''.

Figure \ref{fig-equiv}, from Lakens et al. 2018\footnote{Lakens et al.(2018). Equivalence Testing for Psychological Research: A Tutorial. \textit{Advances in Methods and Practices in Psychological Science}, 1(2): 259--269. \Burl{https://doi.org/10.1177/2515245918770963}}, shows equivalence testing alongside the usual null hypothesis significance testing, as well as other tests. % Which of the four cases represented corresponds to the chlorine example in the notes?
In some cases it is relatively straightforward to test for equivalence (such as with the TOST procedure), in many others it is not.
...
We will not pursue this any further. But, please, think of other examples of similar cases. Examples involving, for example, human disease and certain environmental changes, or secondary effects of drugs, or that two drugs (say, brand and generic) are for all practical purposes identical. What case would represent examining if the effects of the brand name drug and a generic are similar?
...
\item Using Bayes rule to obtain the posterior probability of a hypothesis given data. Why this is a very attractive idea. (Where we say ``the posterior probability of a hypothesis'', we could write ``the posterior probability of a parameter'', or its density, or the parameter taking a particular value ---make sure you can write this!).
...
\item The problems of trying to, e.g., get \(P(\theta = 0|Data)\): where is the prior for \(\theta\) (or the hypothesis) coming from?
...
In this case, the violin plot is an overkill because we have too few data points. This is just to show it as an example. (But recall our critique of violin plots in general).
...
and then rank the within-subject differences; think also about how comparing within-individual differences is not necessarily the same as comparing within-individual ratios; and what do you do when you take the difference of the logarithms?
...
We will use the \Robject{BRCA2.txt} data set. Import it, etc.
...
But we can also call \Rfunction{aggregate} in a simpler way in this case:

<<>>=
aggbrca <- aggregate(brca2 ~ cond + id, data = dbrca, FUN = mean)
@

However, we want to \textbf{double check that we have the exact same number of measures per subject}. How? A simple procedure is to aggregate again (give the output another name) but using ``length'' instead of ``mean''.
...
We can do it in one go, computing the mean and counting:
...
Back to the t-test above. Pay attention to the
degrees of freedom (and the statistic and p-values):

<<ttagg2>>=

## Now, t-test
t.test(brca2[, "Mean"] ~ cond,   data = aggbrca3)

## Why am I accessing the "Mean" component that way?
## Look into the aggbrca3 object.


## That is, of course, the same as:
t.test(brca2 ~ cond, alternative = 'two.sided',  data = aggbrca)

@

In real life, you would automate checking that we have the same number of replicates. How??
...
%% FIXME: reorganize this material. Follow more the structure in
%% https://people.math.ethz.ch/~meier/teaching/anova/contrasts-and-multiple-testing.html
%% So maybe FDR vs FWER first, and then the rest. Or not. But as of now this is slightly disorganized.
...
Again, notice the output says ``simultaneous'': as emphasized in Lukas Meier's ``ANOVA and Mixed Models'' (2025, CRC Press; HTML from \Burl{https://people.math.ethz.ch/~meier/teaching/anova/contrasts-and-multiple-testing.html\#sec-tukey-hsd}), ``these confidence intervals are simultaneous, meaning that the probability that they all cover the corresponding true difference at the same time is 95\%.''.
...
mentioned, is chapter 3 of Meier's book `` ANOVA and Mixed Models: A Short
Intro Using R '', 2025, CRC Press; on-line at  \Burl{https://stat.ethz.ch/~meier/teaching/anova/contrasts-and-multiple-testing.html}.
...
%% En la pizarra, do as follows:
%% First, a 2x3 model.
%% Write additive model, for each cell
%% Then, the interaction. See how many more parameters we need.
%% Precisely as many as taken by df of interaction term
%% Then, show the difference in # params between additive and
%% interaction in the 4x7
...
## A: 4 levels
## B: 7 levels
## Additive: 1 + 3 + 6 = 10 params
## Multipl:  1 + 3 + 6 + (3 * 6) = 1 + 27 = 28: number of cells

## The very first is this
## A: 2 levels
## B: 3 levels
## 1 + 1 + 2 for additive
## 1 + 1 + 2 + (1*2)
## and do a figure and get why we need two deviations
## use, for each cell, things like
## mu
## mu + alpha_2
## mu + beta_2
## mu + beta_3
## mu + alpha_2 + beta_2 + deviation
## mu + alpha_2 + beta_3 + deviation
...
This is a plot of how Y changes with Diet, separately for each level of Drug.
...
The above is a plot of the ``effect''\footnote{Quoted, as effect tends to denote causal relationship, and it need not in an ANOVA.} of switching Drugs (the abscisas) for the different Diets (each of the segments with a different line type).
...
SPSS, SAS, etc.)\footnote{More details about Type I and Type II Sums of Squares are
    provided in \qref{ssI-to-III}, but you can skip it. This
    issue is also discussed, by focusing on the key issues from an applied
    perspective, in chapter 5 of Fox and Weisberg's ``An R companion to applied
...
Again, recall we assume no significant interaction when looking at the main effects.

What is \verb@ SS(A|B) @: the Sums of Squares explained by A (after accounting for B): the model with only B has its residual SS increased by SS(A|B) compared to the model with both A and B.
...
And some numbers here, with an old example:
<<>>=
## Recall this:
## dcholest <- read.table("Cholesterol.txt", header = TRUE)
...
m_drug_diet <- lm(y ~ Drug + Diet, data = dcholest)
...
### Diet when accounting for Drug.
anova(m_diet_drug) ## diet on its own first
anova(m_drug_diet) ## diet after drug
Anova(m_diet_drug) ## Our well know Type II SS.


#### Comparing models and getting the Type II SS
anova(m_drug, m_diet_drug)
## SS(diet|drug) = R(m_drug) - R(m_diet_drug)
## Of course, R(m_diet_drut) = R(m_drug_diet)
## 167.3: RSS from model m_drug
## 91.8 : RSS from model m_drug_diet
## 75.5 = 167.3 - 91.8


#### F-statistic for Diet
## (75.45/2)/(91.81/36) = 14.79
## (SS(diet|drug)/df added by diet)/(R(m_diet_drug)/res.df.m_diet_drug)

### Could do the same with Drug after accounting for Diet
anova(m_drug)
anova(m_diet_drug)
...
But wait!!! In the example above, \textbf{recall we should not be looking at the main effects, as we know the interaction is significant!}. We did that just for the sake of showing where the numbers come from.
...
statistical analysis using R, 2nd ed'', and then the documentation of R package \CRANpkg{multcomp}.
...
\item Finally, in both cases, you might want to think if carrying out comparisons
  ...
  all pairs and reporting only a few is incorrect, leads to biases, and is poor practice and constitutes scientific fraud).
\end{itemize}
...
second will have B in the X axis. You will see that in both plots the lines cross.
...
I will use mostly function \Rfunction{glht} from package \CRANpkg{multcomp}. I will skip most of
the syntax here. Examples using function \Rfunction{mmc} from package \CRANpkg{HH} are provided later (though the figures might need some time to be understood).
...
\Burl{https://cran.r-project.org/web/packages/multcomp/vignettes/multcomp-examples.pdf}). (Ooops, that file is no longer available from the repo, though it can still be found on the internet). %% FIXME: locate it?


<<clmAglh>>=
## We must make sure we are using factors with mmc and
## turning them to factors is good practice anyway
dc2 <- dcholest
...
summary(clmA_glh)
confint(clmA_glh)
## Modify margins to fit the labels
op <- par(mar = c(5, 6, 4, 1))
plot(clmA_glh)
par(op)
@

We can of course only compare, say, Diet, and these are narrower, as expected:
<<clmdglhfig>>=
clmD_glh <- glht(clmA, linfct = KA1)
confint(clmD_glh)
op <- par(mar = c(5, 6, 4, 1))
plot(clmD_glh)
par(op)
...
Using directly \Rfunction{glht} to compare both Diets and Drugs (and notice the warnings):
...
confint(glht(clm, linfct = rbind(K1A, K2A)))


@

We could just compare, say, Diets:
<<gldint2>>=
confint(glht(clm, linfct = rbind(K1A)))
@
...
\Burl{https://cran.r-project.org/web/packages/multcomp/index.html}).
%% FIXME: multcomp-examples.pdf was great for this! no longer available
%% see above
% \Burl{https://cran.r-project.org/web/packages/multcomp/vignettes/multcomp-examples.pdf}). (Ooops, that file is no longer available from the repo, though it can still be found on the internet). %% FIXME: locate it?
...
Note that many of those family-wise confidence intervals can be obtained, too, using the \Rfunction{TukeyHSD}
...
## Compare to, for example, confint(glht(clm, linfct = rbind(K1A)))
TukeyHSD(clmaov, which = "Diet")


## Diet and Drug. Compare to confint(glht(clm, linfct = rbind(K1A, K2A)))
TukeyHSD(clmaov, which = c("Diet", "Drug"))
@


What if we want the comparisons for all possible combinations?

<<tukey2wayintall>>=
## If we want all possible contrasts for all combinations
...
We have 15 contrasts. Maybe we do not care about all those (e.g., \texttt{M1:A-HF:B}). Again, we might only care about all comparisons of Drug within each level of Diet, for example (see documentation of R package \CRANpkg{multcomp}).


%% FIXME: check emmeans. Maybe simpler and cleaner?

And a minor point:
<<tukey2wayhsd>>=
## If we only care about the contrasts
## this model uses a different parameterization, but is identical
## to the one specified as Diet*Drug
...
\subsubsection{Multiple comparisons in two-way ANOVA, examples using package \texttt{HH} and function \texttt{mmc}}

This is provided without much comment, for the sake of giving you additional examples. Compare these examples with what we did before.

First, pretending there is no interaction (compare with \texttt{clmA\_glh}, before):
...
## Note that these are wider than the ones for both Diet and Drug
## with clmA_glh, above, as we should expect.
@
...
## of M1 - HF
confint(glht(clm, linfct = K1))
confint(glht(clm, linfct = K1A))
@
...
\item To clarify the above, think about this: suppose you take a huge sample, say of size 10 million, of people and do a regression of body mass on body height. You will estimate the regression line with very little uncertainty. In other words, you will be very, very certain about where $E[body\_mass]$ is given $body\_height$. But \ldots no matter your sample size, there is quite a bit of variation in body mass for a given body height. So the prediction bands will be relatively wide, to accomodate this fact.
  ...
  This is, by the way, the reason why you can be very certain about an overall trend (the expected value) and yet be unable to really predict any one specific individuals actual value. (This is similar to situation where you can say, for example, that a particular intervention or factor is associated with a change in the probability of event A of 70\%, and yet that might not be a very precise forecast for any specific idividual.)  It is crucial to understand the difference between predicting the mean and predicting a specific individual's value.
...
\label{sec:vari-infl-fact}

In our pemax example we saw several problems (e.g., the large standard errors in the estimated regression coefficients and the flip in signs) caused by the strong linear relationships between predictors. In the simplest cases, these can be detected by plots, as we did. But that  will not always show the source of the problem, specially when the linear relationship involves groups of variables, where pairwise plots can fail to show a clear pattern.

The variance inflation factor, that we can compute with function \Rfunction{vif} (from \CRANpkg{car}) estimates ``the harm produced by collinearity'' (Fox \& Weisberg, ``An R companion to applied regresssion, 3rd ed'', section 8.8):

<<vif1>>=
vif(mcyst)
@

We can see that the standard error for age is about \(\sqrt{8} = 2.8\), so almost three times larger than it would have been without collinearity.

Compare this with a very different model, where we only have age and sex:

<<vif2>>=
mcystas <- lm(pemax ~ age + sex, data = cystfibr2)
vif(mcystas)
@
which is not surprising, since the data collection is fairly balanced regarding age and sex of the subjects.

Variance inflation factors should be part of your standard modeling and diagnostic tools.

There are no hard cutoffs. Any VIF larger than 1 indicates that, well, there is harm being doing by collinearity. Values larger than 10 are often regarded as very serious collinearity problems. But values larger than 4 already signal a problem: the standard error is twice as large as it would be without collinearity.


For terms with multiple degrees of freedom (factors with more than two levels, polynomials, splines, etc), a modified VIF needs to be used, That is provided by the \Rfunction{vif} function in \CRANpkg{car}, too. See details in the help and in Fox \& Weisberg's book.




\subsection{Interactions between predictor  variables in multiple regression}\label{regr-int}
...
\item There is not relationship between Y and X, but the means differ.
...
ANCOVA is a procedure for differentiating the above scenarios. It also works when the discrete group has more than two levels. And, for that matter, we can deal with more than one discrete variable and more than one continuous one, etc, etc: these are just linear models. As said several times already, we will use the name ANCOVA as this might be familiar. But once you understand the process, these are just linear models that allow to flexibly model scientific problems even if they no longer fit in the simple ANCOVA framework.

To put it differently, these are the steps:
\begin{enumerate}
\item \verb@ Y ~ X * Z@. Look at the interaction first (as we \textbf{always do})
\item If there is no interaction (no different slopes): fit \verb@ Y ~ X + Z@:
  \begin{enumerate}
  \item If X is "not-significant": we are left with \verb@ Y ~ Z @ (different means)
  \item If Z is "not-significant": we are left with \verb@ Y ~ X @ (same slope, single intercept, so same mean conditioned on X)
  \item Neither X nor Z are significant: single mean for Y.
  \end{enumerate}
\end{enumerate}

(I wrote ``not-significant'': use sensible criteria, not just blindly focus on p-values).
...
\subsection{Another path in the fork: different means}
\label{sec:another-path-fork:}


<<anco3gen,echo=FALSE, results='hide'>>=

set.seed(1)
X <- runif(n = 40, min = 5, max = 20)
Z <- sample(c("a", "b", "c"), size = 40, replace = TRUE, prob = c(1, 1, 1))
Y <- ifelse(Z == "a", 1, ifelse(Z == "b", 2, 3))
Y <- Y + rnorm(40)
anco3 <- data.frame(Y = round(Y, 2), X = round(X, 2), Z = Z)
rm(X, Y, Z)
write.table(file = "anco3.txt", anco3, row.names = FALSE, quote = FALSE,
            sep = "\t")
@


<<anco3>>=
anco3 <- read.table("anco3.txt", header = TRUE, sep = "\t")
anco3$Z <- factor(anco3$Z)

mfull <- lm(Y ~ X * Z, data = anco3)
Anova(mfull)

madd <- lm(Y ~ X + Z, data = anco3)
Anova(madd)

summary(madd)

scatterplot(Y ~ X | Z, data = anco3, smooth = FALSE, by.groups = TRUE)

## or use ancova from HH,  but note these are sequential (not Type II)
## SS
ancova(Y ~ X * Z, data = anco3)
@


Note, though, that in some cases, when X is a pre-specified covariate, it might be sensible to keep it in the model regardless of p-value. This would be the case if the purpose is ``compare Z, adjusting for X''.
...
\subsection{More paths in the fork}
\label{sec:more-paths-fork}

Of course, things can get more complicated: say there are four groups. Two of them could have the same slope, and the other two groups different slopes. Etc.
...
\subsection{A complication to discuss in class}
\label{sec:compl-disc-class}

Suppose the range of X is very different between the Z groups: what if your model finds an interaction? Is this really an interaction, or is the relationship between X and Y different in different parts of the range of X?
...
<<qqPlotexample, echo=FALSE, results='hide', eval=FALSE>>=
set.seed(1)
sex <- factor(rep(c("Male", "Female"), c(20, 20)))
drug <- factor(rep(rep(c("A", "B"), c(10, 10)), 2))
y <- rep(c(8, 16, 10, 12), rep(10, 4))
...
## What is the qqplot on the upper left doing?
## verify:
par(mfrow = c(1, 2))
plot(myInteract2, which = 2)
qqnorm(rstandard(myInteract2))
qqline(rstandard(myInteract2))

library(car)
qqPlot(myInteract2, reps = 2000)
plot(myInteract2, which = 2)

## Could also do
plot(myAdditive2, which = 2)
plot(myAdditive2b, which = 2)

## But here look at the envelope for the outlier one
qqPlot(myAdditive2, reps = 5000)

## Without the offending
qqPlot(lm(y ~ sex + drug, data = y.data2[-1, ]), reps = 5000)
...
\item ``Component+Residual'' (or partial residual\footnote{These plots,
    ...
    horizontal axis and, on the ordinates, the ``partial residuals'' which are the values of the response with the effect of all other predictors removed, i.e., $=
    residual + \hat{\beta}x = y - \sum_{\mbox{o=all other predictors}} \hat{\beta_o} x_o$. See, for example, Faraway, ``Linear models with R, 2nd ed'', p.~94 and ff. and Fox \& Weisberg, ``An R companion to applied regression, 3rd ed.'', section 8.4.2}) plots allow us to
...
\item Variance inflation factors help us detect possible problems caused
  by collinearity (correlations between independent variables). We saw them in \qref{sec:vari-infl-fact}.
...
% For me: partial and added variable residuals seems similar but are not identical
% In the added variable, we regress Y on all others.
% In the partical residual, we regress Y on ALL, and put back the predicted from the affected one. Or, alternatively, compute the residual as
% Y - the x*beta, where the betas are from the model with all. This shows an example (but this ain't extreme, as no collinearities, etc)

% x1 <- runif(100)
% x2 <- runif(100)
% x3 <- runif(100)
% y <- x1 + 2*x2 + 3* x3 + rnorm(100)
% df44 <- data.frame(y =y, x1 = x1, x2 = x2, x3 = x3)
% m44 <- lm(y ~ x1 + x2 + x3, data = df44)
% pr44 <- residuals(m44, type = "partial")
% m44_r23 <- residuals(lm(y ~ x2 + x3, data = df44))
% m44_r12 <- residuals(lm(y ~ x2 + x1, data = df44))
% plot(pr44[, 1], m44_r23)
% plot(pr44[, 3], m44_r12)
...
Now, suppose we have a bunch of models, and we measure the AIC of each. Model selection via AIC tells us to prefer the model with the largest AIC. (Note: largest if we define AIC as above; we should prefer the model with the smallest AIC if we define AIC as
$AIC = -2 \ln(\hat{L}) + 2\ k$).
...
Notice that \texttt{step} adds or drops one variable at a time. \texttt{step} will not compare the AIC of all possible models. Thus, it could happen that ``the best'' model is never found if there is no path made up of ``drop or add one variable at a time'' from the initial to the best model \footnote{Suppose a model of Y on five variables. Suppose the best model is Y with the first two variables, with an AIC of 1; suppose that all three variable models have an AIC of 3, suppose the model on the first four variables has an AIC of 1.5 and suppose the initial model on all five variables has an AIC of 4. Starting from this last model, you don't reach the best model.}. You could, if you want, write code to compute the AIC of all models (if the number of models is reasonable small or you have infinite patience).
...
conducting statistical hypothesis testing}\footnote{The hypotheses tests we conduct with Anova are, in fact, comparisons of models, where we test the null hypothesis that the smaller model is good enough; equivalently, that the larger model does not make any relevant difference (i.e., reduce RSS enough).}. In contrast, the AIC criterion is not
...
\section{Experimental design: a reminder of its importance}
...
from Curran-Everett, 2013\footnote{Curran-Everett, D. (2013). Explorations in statistics: The analysis of ratios and normalized data. \textit{Advances in Physiology Education}, 37(3), 213-219. \Burl{https://doi.org/10.1152/advan.00053.2013}.}.Before we start, though, note that this section should not really be   necessary after previous sections :-).




We will pretend there is a response variable, called ``Y'', two groups (``g1'' and ``g2'')
and another variable, called ``X''. X might be some reporter protein, or
something that can be taken as a proxy for cell volume, etc.

This is a table from Curran-Everett's paper with possible ``Y'' and ``X'':


\begin{figure}[h!]
  \hspace*{-45pt}\includegraphics[width=0.80\paperwidth,keepaspectratio]{curran-everett-f1.png}
  \caption{\label{curran} Table 1 from Curran-Everett, D. (2013). Explorations in statistics: The analysis of ratios and normalized data. \textit{Advances in Physiology Education}, 37(3), 213-219. \Burl{https://doi.org/10.1152/advan.00053.2013}.}
...
that way (the code is below; look at it if you want).

<<>>=
set.seed(1234) ## irrelevant, but so that we all
...
y1 <-  2 * x1 + 3 * as.numeric(t1) + rnorm(2 * n, 0, sd)
data1 <- data.frame(Y = y1, X = x1, Group = t1, Ratio = y1/x1)
@
...
with(data1, plot(Y ~ X, col = c("red", "blue")[Group]))
abline(lm(Y ~ X, subset(data1, Group == "g1")), col = "red")
abline(lm(Y ~ X, subset(data1, Group == "g2")), col = "blue")
...
\clearpage
<<>>=
summary(lm(Y ~ X * Group, data = data1))
Anova(lm(Y ~ X * Group, data = data1))
...
anova(lm(Ratio ~ Group, data = data1))
@
...
in the rate of change of Y relative to X, but the groups differ in
intercept. As you can see, the analysis with ratios is misleading.



Recall this is what we are doing with the ratios: computing the line through the origin for each point, and averaging all those for each group.
...
set.seed(123)
sd <- 0.1
x2 <- seq(from = 1, to = 3, length.out = n)
ya <- x2 + rnorm(n, 0, sd)
yb <- 0.5 * x2 + 1 + rnorm(n, 0, sd)
y <- c(ya, yb)
tf <- factor(rep(c("g1", "g2"), rep(n, 2)))
x <- rep(x2, 2)
data2 <- data.frame(Y = y, X = x, Group = tf, Ratio = y/x)
@
...
with(data2, plot(Y ~ X, col = c("red", "blue")[Group]))
abline(lm(Y ~ X, subset(data2, Group == "g1")), col = "red")
abline(lm(Y ~ X, subset(data2, Group == "g2")), col = "blue")
legend(x = 1.5, y = 2.5, legend = c("g1", "g2"), col = c("red", "blue"),
       pch = 1)
@

<<>>=
summary(lm(Y ~ X * Group, data = data2))
...
of change of Y relative to X differs between the two groups. But just
