...
\item \ldots  there was this additional complicating factor of the 1\%:  is it not simply \(H_0: \mu_{chlorine} \ge \mu_{control}\);
  our null is \(H_0: \mu_{chlorine} \ge \mu_{control} + 1\% \)
\end{itemize}


The above would be an example of an ``inferiority'' test (there are also ``non-inferiority'' tests, tha test the opposite).
...
What case would represent examining if the effects of the brand name drug and a generic are similar? In some cases it is relatively straightforward to do this (such as with the TOST procedure), in many others it is not.
