...
\item What a statistic is; estimators (for example, the sample mean is an
  estimator) are a type of statistic \footnote{Briefly: a statistic is
    ...
    distribution using the data from a given sample. Thus, all estimators are statistics (or: estimator = statistic + specified target parameter), whereas not all statistics are necessarily estimators, in the sense that we don't use them to estimate a particular parameter; for example, we often use the t-statistic, as we will do in this class, as a statistic for testing, without mapping it to the parameter of anything ---though it could be, as the non-centrality parameter of a t-distribution}.
