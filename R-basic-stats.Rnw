...
Here is a interesting and recent example where the winner's curse can be
  ...
  such as reporting/publication bias and the reproducibility crisis.).
    ...
    Final note when we conduct many hypotheses tests (as in screening): using control of multiple testing (that we will see later) \textbf{does not} minimize or prevent the winner's curse. Actually, the opposite.
