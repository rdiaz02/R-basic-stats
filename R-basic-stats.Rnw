...
%% FIXME: this is poorly explained and two issues
%% a) sometimes we want something in the null or the alternative
%% b) bioequivalence per se.
%% the example here with the fish is NOT bioequivalence.
%% We could set the null to be ''mortality > 1%'' and thus alternative
%% mortality < 1, and ask for strong evidence against that null.
...
\red{FIXME: this is poorly explained and I must clarify two issues:}

\begin{enumerate}
\item sometimes we want something in the null or the alternative; the null might not need to be ``nothing happens'' in depending what meaning or context
  \item bioequivalence per se
\end{enumerate}

The example here with the fish is NOT bioequivalence. The example with the fish illustrates that we might, from precautionary arguments, say: the null must be  ``mortality > 1\%'' and we ask for strong evidence against that null before allowing for chlorine dumping.

So start with the fish, and then explain bioequivalence.
...
\externalf Make sure  to look at the notes you took for the file
...
\section{FIXME: one vs two-tailed tests}
...
\subsection{What a two-sample Wilcoxon is not}
...
differences (discarding the sign) are then ranked, and then the sum of the
...
Even if this procedure is (or was) common in some circles, you now know why it is
a bad idea.  Please, do not do this.
...
% ANOVA. Read \textbf{now} the 2 and a half pages from Peter Dalgaard's book available in
...
\externalf Read \textbf{now} the 2 and a half pages from Peter Dalgaard's book available in
...
Note that, in our usage of Tukey, we did not pre-specify that
...
would be rejected.''  This might sound like a mess, but it really
ain't. Think about it. To get going, start reading the sentence covering
...
p-values case. It is so nice that it makes comparisons very simple, as
Wright explains: ``An adjusted $P$-value can be compared directly with any
chosen significance level $\alpha$: If the adjusted $P$-value is less than
or equal to $\alpha$, the hypothesis is rejected.'' (If you find this too advanced for now, don't worry. But remember this once you feel more comfortable with these issues. And no, this paragraph will not be in the exam.)
...
Briefly: try not to do this. You will not be able to test interactions. And, of course \ldots that would be a tiny sample size anyway.
+
...
\subsubsection{Comparisons between means when there are interactions: a simple
...
sense.  Think about changing the slope of the regression line, keeping the center
...
\begin{itemize}
\item $R^2$ is the proportion of variability in the dependent variable accounted for (or explained) by the model. It is also the square of the correlation between observed and predicted (predicted according to the model) values of the dependent variable.
...
The following shows the calculations but the previous summary is enough for this class.
...
How do we interpret coefficients? These are a few hints. Suppose we have\\
...
And a simple numerical example:
<<>>=
...
<<f2d77z,echo=FALSE, fig.pos = '!h',fig.width=5.8, fig.height=5.8, fig.cap='', fig.lp="fig:", fig.cap='Yet another example of non-constant variance (plus other issues)'>>=
...
Interpretation?  Manipulation? Variable selection is a touchy and delicate
...
a biological point of view, or causally, etc. This is a very quick summary:
...
\item Prediction: predict a variable given others. This might be totally unrelated to intervention (which is often what causal inference is after) or understanding/gaining insight into a process.
  ...
  Many models are of this kind. For example, many meteorological models are like this: if you see AEMET's predictions you only care about them being right (and they often are). You are not trying to modify anything (i.e., ``let's try to make of today a rainy day'') nor understand; and this is why purely phenomenological models of high complexity, including deep learning, with millions of parameters that our brains can't just wrap their heads around, are just fine for these tasks.

  
\item Interpretation/understanding/insight: why are things this way? What factors are relevant for a process and why? This is trying to gain scientific understanding. Is sleep deprivation what causes whatever? Or is it differences in light exposure? Or ... This is often what scientific modeling is about.
...
(in all their variants, such as stepwise, etc). These procedures rarely do what you want to do, are often extremely unstable, etc. They are trivial to implement, and that makes them very common, but they are rarely what you scientific question needs.
...
When we compute the sums of squares for interaction we first fit a model with an overall (or common) mean, and rows and columns effects and then estimate the deviations of all cells from it to fund the sums of squares. In other words, the deviations that are squared and then summed are the deviations of all the observed cell means from a model with a common mean (the overall mean) and rows and columns effects (the marginal means) that combine additively. We are computing the variation due to interaction, the variation in cell means that deviates from the additive model.  In our case, when we computed the SS each of those cell deviations terms were $0.75$ or $-0.75$. 
