...
Here is a interesting and recent example where the winner's curse can be
  playing a role: the possible use of botox as an antidepressant treatment   \Burl{https://www.science.org/news/2021/06/can-botox-ease-depression-eliminating-frowns-researchers-have-doubts}.
...
\subsubsection{Several ways of obtaining summaries}
...
\subsubsection{Can you do an ANOVA with only one sample per group?}\label{anova-only-one}
Why or why not?
...
\subsection{Fitting a two-way ANOVA: cholesterol}\label{twoway-chol}
...
\item ``Component+Residual'' (or partial residual\footnote{These plots,
    for each variable, show a plot of the independent variable, say $x$, on the
