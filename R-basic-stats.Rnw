...
This work is Copyright, \copyright, 2014-2021, Ramon Diaz-Uriarte,
...
though, is material you have surely been taught before :-)
...
available as ``central limit theorem''. There is also code in the repo to
directly play with the central limit theorem; see file \texttt{central-limit-theorem-ex.R}).
...
\subsection{Confidence intervals and p-values: a longer discussion}

\red{Make sure you remember to look at the notes you took for the file
  \texttt{confidence-intervales-p-values-interpretation.pdf}}.
...
\item The threshold we use for saying ``the means differ'' ($\alpha$ level, or
  Type I error). (Note: remember to ask me to explain the differences between
  $\alpha$ level and Type I error and p-value).
...
\red{If the trade-off between Type I and Type II errors is not clear, remember
...
\subsubsection{Reshaping et al: final comments}
...
ANOVA. Read now the 2 and a half pages from Peter Dalgaard's book available in
...
%% \red{zz: FIXME} the next seems out of place. Look at the BM-1 notes, where
%% I show this as ``Anova tables from lm et al'' and we reuse a simple
%% example from cholesterlo, which makes for easier following.
...
or equal to $\alpha$, the hypothesis is rejected.''
...
We will explain it in more detail later (\ref{anovaaslm}), but for now
...
%% said, we will get back to this later (\ref{anovaaslm}).
...
multiple regression et al. in section \ref{regr-int}).
...
13, Number 2, pp. 163-167, or the many debates about Type III SS. There
...
this depends on how the interactions are, and what your scientific question is.
...
How do we interpret coefficients? Just a few hints here. Suppose we have\\
$y = \alpha + \beta_1 x_1 + \beta_2 x_22 + \beta_{1,2}\ x_1\ x_2 + \epsilon$ (where the
term $\beta_{12}\ x_1\ x_2$ is literally the product of $\beta_{1,2}$ times the product of
$x_1$ and $x_2$). Now, in a table of coefficients, what is the meaning of
$\beta_1$? That is a table that ``shows every variable when everything else is
also in the model'' (and that includes the interaction).  You can think of
$\beta_1$ as just the best term that solves the above equation ($y =
\alpha + \beta_1 x_1 + \beta_2 x_22 + \beta_{1,2} x_1 x_2 +
\epsilon$). But think about the interpretation of ``a coefficient tells me
how fast the response changes when the predictor changes by one
unit''. Let's do that: $\frac{\partial y}{\partial x_1} = \beta_1 +
\beta_{1,2}\ x_2$; see how $x_2$ is there. How fast $y$ changes with $x_1$
is also a function of $x_2$. 
...
(Note the warning: the predictions should not be used to assess how well the
model predicts the data used for the fitting itself.)
...
If we go back to \ref{simpletwo} we can also do the same and change the
...
What if we go to the data in section \ref{simpletwo}? Again, in the
...
\subsection{Other contrasts}\label{othercontrasts}
...
There is nothing conceptually new here. Go back to section \ref{anovaaslm}
...
explained in section \ref{diagnostics} to choose among models. But, in
...
design, the data used in section \ref{ordermatter}. The diagnostic plots
...
\Robject{cholestanova}, from section \ref{twoway-chol}, looks relatively OK
...
relatively so little data\footnote{Again, this is an interesting case, since
...
\subsection{Diagnostics: a couple more examples with designed experiments}
...
\Rfunction{step} function) and even better is then bootstrapping the whole process
to get estimates of error, predictive ability, etc. Even better, of
course, is subject-matter guidance on how to proceed and what are and are
not candidates for deletion and in what order. The book by Frank Harrell,
``Regression modeling strategies'' contains great discussions of these
topics. The actual data set used here is discussed, also in the context of
variable selection, by P.\ Dalgaard in chapter 11 of ``Introductory
statistics with R''.

Of course, using model diagnostics (section \ref{diagnostics}) with the
...
%% Something is fucked up in the processing, as commenting or uncommenting
%% some of the above leads to a non-compiling tex. Go figure. It seems
%% some kind of interaction between knitr and something else. So leave the
%% comments in the last chunk.
...
\ref{wrong}, as this is probably not a sensible model)
