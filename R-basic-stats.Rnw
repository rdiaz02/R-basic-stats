...
Again, notice that we have 15 contrasts. Maybe we do not care about all those (e.g., \texttt{M1:A-HF:B}). We might only care about all comparisons of Drug within each level of Diet, for example (see
\Burl{https://cran.r-project.org/web/packages/multcomp/vignettes/multcomp-examples.pdf}).
