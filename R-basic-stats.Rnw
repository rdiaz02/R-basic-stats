...
it. And yes, we have not mentioned Type III, though I give a piece of
output in section \ref{regr-int}).
...
I take this from \Burl{http://www.utstat.utoronto.ca/reid/sta442f/2009/typeSS.pdf}:
...
in this notes. (Search the paper by Bill Venables ``Exegeses on linear
...
13, Number 2, pp. 163-167, or the many debates about Type III SS.)
...
last term is literally the product of $\beta_{1,2}$ times the product of
$x_1$ and $x_2$). Now, in a table of coefficients, what is the meaning of
$\beta_1$? That is a table that ``shows everything when everything else is
...
\beta_{1,2}\ x_2$; see how $x_2$ is there. This contrasts with the model without interaction:
...
applies not only to Ancova, but to linear models in general\footnote{Type
  ...
  there is an interaction term in the model.}.
...
To give some examples (look at them carefully, and compare with what we
did by hand, and with whether or not these are the models you would use):
