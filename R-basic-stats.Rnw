...
three things from a four-means design.  How do we interpret the
...
here, but that is not just the first cell; see below if you want).
...
We can change the reference, and that changes what we are comparing. For
instance, let us recode A and run the model again:
...
\clearpage
\section{Anova tables from \Rfunction{lm} et al.}\label{anovaaslm}
