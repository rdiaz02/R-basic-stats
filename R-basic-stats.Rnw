...
bunch of two-by-two boxes with the sample sizes in each cell):
...
Some plots to try to understand  the above patterns (these are just some initial steps):
...
\subsection{A large pemax model}
