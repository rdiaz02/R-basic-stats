...
% \Burl{https://github.com/rdiaz02/BM-1}.
(Note that in the
...
coordinates(ptr22) <- list(x = c(ChildhoodBMI = 1,
                                 Exercise = 2,
                                 Outcome = 3,
                                 U_g = 0.8,
                                 U_c = 0.8),
...
You know the answer. No, do not adjust for L (Childhood BMI): it is a collider ($U_1$ and $U_2$
...
And regardless of whether you work with observational or experimental data, the causal framework is the appropriate framework to think about confounding, as we have seen above. This cannot be emphasized strongly enough, and if you have doubts, look at the example in \qref{pretreat-no}.
...
\citealp{morgan_counterfactuals_2015}; \citealp{neal_causality_2020}, \citealp{peters2017}).
...
the family of the persons being hired (this is from \citealp{pearl_causal_2016}, Fig.~312, p.~77, ---but  I have substituted ``Income'' byt ``SocioecStatus'').
...
biased because Qualification is a collider (a descendant of both SocioecStatus and
...
Finally, for figure \ref{fig:dag_direct_conf}, \citealp{pearl_causal_2016} discuss the difference between conditioning on ``Qualification'' and \textit{fixing it constant} by \textit{intervening} on it, which allows us to compute the \textit{Controlled Direct Effect (CDE)} (the change in Hiring by intervening on both Gender and Qualification, where the presence of SocioecStatus no longer bothers us, since we are manipulation Qualification); we will not discuss this further; \citet{morgan_counterfactuals_2015} explain them in ch.~11, and \citet{hernan2020} on Technical point 22.1 and sections 23.2 and ff.


Of course, you can turn the examples above into a completely different example that could be easier to relate to. For example, with scarce display of imagination on my side:
\begin{itemize}
\item Gender is Gene A, whose expression you can manipulate.
\item Qualification is Gene B
\item SocioecStatus is Gene C
\item Hiring is some desired outcome
\end{itemize}

Please, make this more elaborate (e.g., think about some of the variables above being, say, a particular therapy, etc).
...
\citet{pearl_causal_2016} or in section ``Fine point 6.1'' of
...
\citet{pearl2016} (and \citealp{woodward_making_2003} and \citealp[][ch.~5]{otsuka2023} for philosophical discussion). (\citealp{rosenbaum2017}, does not use DAGs at all; the others
...
experimental data. In fact, you most likely think counterfactually (pun intended) a lot of the time: would you have had so much fun for the last hour or so had you not been reading these notes? \smiley{}
