...
Commons license) from \Burl{https://github.com/rdiaz02/R-basic-stats}. (Note that in the
...
manipulating X will change the value of Y.
...
\subsection{There is no such thing as  ``spurious associations''. But when people
  use this term, it suggests they are trying to obtain causal estimates}
...
\citet[][p.\ 84]{hernan2020} for similar comments.
...
The Appendix is optional (though not harmful).
...
variable, or variables, are not affected by treatment). The DAG is shown in \ref{fig:plot_fungus_1}
...
have been good, but we can't; anyway, not including U in the model will increase
...
Put it differently, in the chain example in Figure \ref{fig:plot_dag_struct} a),
...
the ``in addition''. The \textbf{most important structure to pay attention to is
  f)}. The rest is here for completeness, but following it actually only requires
...
You know the answer. No, do not adjust for L: it is a collider ($U_1$ and $U_2$
...
it, Z is not being used as an instrument.}.
...
\citealp{morgan_counterfactuals_2015}; \citealp{neal_causality_2020}).
...
\clearpage
\setcounter{part}{1}
\part{Appendix and additional material}
As it says: what follows gives additional details on some issues above. 
...
what are called backdoor paths.  For completeness, here are the details.
...
term d-separation in one footnote in their 490 pages book, though their
procedure, which we have copied above, actually implicitly uses the definition of
d-separation). Anyway, d-separation is well explained in Definition 2.4.1, p.\ 46
of \citet{pearl_causal_2016} or in section ``Fine point 6.1'', p. 76 of
\citet{hernan2020} and is very useful if you want to find all the independencies
and conditional independencies implied by a DAG.
...
\subsection{Randomization and exchangeability}
...
\subsection{Terminology: exchangeability, ignorability}\label{ignorab}
...
\subsection{Randomization, exchangeability, counterfactuals, and potential
...
\citet{pearl2016}. (\citealp{rosenbaum2017}, does not use DAGs at all; the others
do, and draw ---pun intended--- connections between counterfactuals and
DAGs). Key message: even if you don't care about DAGs or making causal inferences
